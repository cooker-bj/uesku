$(function(){
          ...
          $("#comment_list").on('click','span.comment_commands a[href=""]',function(e){
              e.preventDefault();
              $("<div>").html("请点击<a href='/users/sign_in'>登录</a>回复").dialog({modal: true});
    ...
    $("#comment_list").on('click','span.comment_commands a[href="#"]',function(e){
        ...
        {var dialog=$(selector).removeClass('undisplay').dialog({modal: true,
            height: 200,
             title:'回复'});
    ...
    $('.user').on("mouseenter",".pop_out",function(){
        ...
        {
            var dialog=handler.removeClass('undisplay').diaglog({modal:true,position:dp});
